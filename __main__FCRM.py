...
class TakagiSugeno:
    ...
    def find_y(self, path):
        ...
        number, dim = data.shape
        X_data = data[:, :-1]
        X = np.ones(shape = (number, dim))
        for i in range(len(X)):
            X[i][1:] = X_data[i]
        Y = data[:, -1].reshape(1, number)
        All_new_Y = []
        RMSE = 0
        for j in range(number):
            ...
            for i in range(self.clustering.c):
                ...
                for k in range(dim - 1):
                    P1 = self.params[k*self.clustering.c + i][0]
                    P2 = self.params[k*self.clustering.c + i][1]
                    y_tmp.append(gaussmf(X[j][k+1], [P2, P1]))
                w = min(y_tmp)
                sum1 += np.dot(X[j], self.clustering.clusters[i]) * w
                sum2 += w
            New_Y = sum1/sum2
            RMSE += (Y[0][j] - New_Y)**2
        ...
        plt.plot(Xs, All_new_Y, label = 'Cluster: '+str(1))
        plt.plot(Xs, Y[0], label = 'Cluster: '+str(2))
        plt.show()
...
##    E = 0.00005
    ...
    F_test = input("input test data file name: ")
##    F_test = "data_02.csv"
